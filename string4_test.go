...
func (e ErrInvalidRune) Error() string {
	return fmt.Sprintf("invalid utf8 rune at offset: %d", e.Offset)
}
...
func NewString4(s string) (*String4, error) {
	if len(s) == 0 {
		return new(String4), nil
	}
	...
	for {
		...
		if r < utf8.RuneSelf {
			...
			return &String4{buffer: buffer, offsets: offsets}, nil
		}

		if sz = utf8.RuneLen(r); sz > 0 {
			...
			return &String4{buffer: buffer, offsets: offsets}, nil
			//			}
			//			return nil, ErrInvalidRune{Offset: offset - sz}
		}
		return nil, ErrInvalidRune{Offset: offset}
	}
}

// Bytes returns the entire slice of bytes that encode all characters in the
// String.
func (s *String4) Bytes() []byte {
	return s.buffer
}

// Char returns the slice of bytes that encode the Ith character.
func (s *String4) Char(i int) []byte {
...
func (s *String4) Slice(i, j int) []byte {
	if i < 0 || i >= len(s.offsets) {
		return nil
	}
	istart := s.offsets[i]
...
func TestString4(t *testing.T) {
	...
	t.Run("characters", func(t *testing.T) {
		t.Run("empty string", func(t *testing.T) {
			...
			got, want := s.characters(), [][]byte(nil)
		...
		t.Run("a", func(t *testing.T) {
			...
			got, want := s.characters(), [][]byte{[]byte{'a'}}
			ensureSlicesOfByteSlicesMatch(t, got, want)
		})
		...
		t.Run("﷽", func(t *testing.T) {
			...
			got := s.characters()
			want := [][]byte{[]byte{239, 183}, []byte{189}}
			ensureSlicesOfByteSlicesMatch(t, got, want)
		})
		...
		t.Run("ﷹ", func(t *testing.T) {
			...
			got := s.characters()
			want := [][]byte{[]byte{239, 183}, []byte{185}}
			ensureSlicesOfByteSlicesMatch(t, got, want)
		})
	})
	...
	t.Run("benchString", func(t *testing.T) {
		_, err := NewString4(benchString)
		if err != nil {
			t.Fatal(err)
		}
	})
